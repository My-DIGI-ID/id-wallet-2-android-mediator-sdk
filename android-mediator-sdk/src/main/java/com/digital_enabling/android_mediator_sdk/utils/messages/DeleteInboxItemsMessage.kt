...
@Serializable
class DeleteInboxItemsMessage : AgentMessage {
    constructor(useMessageTypesHttps: Boolean = false) : super(useMessageTypesHttps) {
        id = UUID.randomUUID().toString()
        type =
            if (useMessageTypesHttps) MessageTypesHttps.DELETE_INBOX_ITEMS else MessageTypes.DELETE_INBOX_ITEMS
    }

    @SerialName("InboxItemIds")
    var inboxItemIds: List<String>? = null
